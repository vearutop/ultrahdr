...
func (h *huffmanLUT) init(s HuffmanSpec) {
	// Always allocate 256 entries to cover all possible symbols.
	*h = make([]uint32, 256)
	code, k := uint32(0), 0
...
type encoder struct {
	...
	// huffLUT are compiled Huffman tables.
	huffLUT [nHuffIndex]huffmanLUT
	// huffSpec are the Huffman specs used when writing DHT.
...
func (e *encoder) writeDQTSeparate() {
	for i := range e.quant {
		const markerlen = 2 + 1 + blockSize
		e.writeMarkerHeader(dqtMarker, markerlen)
		e.writeByte(uint8(i))
		e.write(e.quant[i][:])
	}
...
func (e *encoder) writeSOF0(size image.Point, nComponent int) {
	...
	if nComponent == 1 {
		...
		for i := 0; i < nComponent; i++ {
			...
			if e.useSampling {
				sf := e.sampling[i]
				e.buf[3*i+7] = (sf.H << 4) | sf.V
			} else {
				// Default to 4:2:0 chroma subsampling.
...
func (e *encoder) writeDHTSeparate(nComponent int) {
	specs := theHuffmanSpec[:]
	if e.useCustomHuff {
		specs = e.huffSpec[:]
	}
...
// EncoderOptions provides custom tables and sampling factors.
type EncoderOptions struct {
	Quality        int
	UseQuantTables bool
	QuantTables    [nQuantIndex][blockSize]byte
	UseHuffman     bool
	HuffmanTables  [nHuffIndex]HuffmanSpec
	UseSampling    bool
	Sampling       [3]SamplingFactor
	SplitDQT       bool
	SplitDHT       bool
...
func initDefaultEncoder(e *encoder, o *Options) {
	...
	for i := range e.huffLUT {
		e.huffLUT[i] = theHuffmanLUT[i]
		e.huffSpec[i] = theHuffmanSpec[i]
	}
}
...
func initEncoderWithOptions(e *encoder, o EncoderOptions) {
	...
	if o.UseHuffman {
		e.useCustomHuff = true
		for i := range o.HuffmanTables {
			spec := o.HuffmanTables[i]
			if len(spec.Value) == 0 {
				// If chroma tables are missing, reuse luminance tables.
				if i == int(huffIndexChrominanceDC) {
					spec = o.HuffmanTables[huffIndexLuminanceDC]
				} else if i == int(huffIndexChrominanceAC) {
					spec = o.HuffmanTables[huffIndexLuminanceAC]
				} else {
					spec = theHuffmanSpec[i]
				}
			}
			e.huffSpec[i] = spec
			e.huffLUT[i].init(spec)
		}
	}
	if o.UseSampling {
		e.useSampling = true
